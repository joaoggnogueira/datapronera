...
class Ctrl_conta extends CI_Controller {
	...
	function password_reset() {
		...
		if ($id = $this->conta->update_password(
				$this->encrypt->decode($encrypted_cpf),
				md5($this->input->post('senha_atual')),
				$this->input->post('nova_senha')
		   ) )
		{
	...
	function email_reset() {
		...
		if ( $this->conta->update_email(
				$this->encrypt->decode($encrypted_cpf),
				md5($this->input->post('senha_atual_email')),
				$this->input->post('novo_email')
		   ) )
		{
