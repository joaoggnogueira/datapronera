...
class Parceiro_m extends CI_Model {
    ...
    function get_record($id) {
        $this->db->select('p.*, pp.realizacao, pp.certificacao, pp.gestao, pp.outros, pp.complemento parc_complemento,pp.demandante, pp.colegiado');
    ...
    function add_record($parceiro) {
        if (($this->db->insert('parceiro', $parceiro) != null) && ($this->db->affected_rows() > 0)
        ) {
    ...
    function add_record_tipo_parceria($tipo) {
        if (($this->db->insert('parceiro_parceria', $tipo) != null) && ($this->db->affected_rows() > 0)
        ) {
            return true;
        } else {
            return false;
    ...
    function delete_record($id) {
        ...
        if (($this->db->delete('parceiro') != null) && ($this->db->affected_rows() > 0)
        ) {
            return true;
        } else {
            return false;
