...
//include('../../../static/sys_stc_var_config.php');
...
class barChart {
	...
	// private $CLASS_PATH = '/home/rubendebest/Dropbox/web_unesp/php/phpexcel/Classes';

	private $_chart_data = 	array(array('',''));//Empty data
	private $_chartType = '';
	private $_legend='right';
	...
	private $_direction = 'col'; //bar | col
	...
	public function set_num_legend_columns($col) {	
		$this->_num_legend_columns = $col;
	}

	public function set_legend_col($col) {	
		$this->_legend_col = $col;
	}
	...
	public function __construct()
	{
		...
		// set_include_path($this->CLASS_PATH); //change this
		//set_include_path(getcwd(). PATH_SEPARATOR . '../');
		
		set_include_path(ROOT_DIR . 'system/phpexcel/Classes'); //change this
	...
	public function create_chart()
	{
		...
		//Legend Col (gambiarra, certo seria arrumar :P )
		...
		for ($cont = 0; $cont < $numDataColumns; $cont++)
		{
			...
			{
				$aux = $cont2+2;
				//$this->_objPHPExcel->getActiveSheet()->getStyle("$contChar$aux")->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)');	
		...
		//	Data values
		//	Data Marker
			$dataseriesLabels[] = new PHPExcel_Chart_DataSeriesValues('String','Worksheet!$' . $contChar . '$' . $legendRow,
				null,
				1,
				array(),
				null,
				// $this->_chart_colors[$cont]
				(count($this->_chart_colors) > $cont ? $this->_chart_colors[$cont] : null)
			);
		}
		...
		$xAxisTickValues = array(
			new PHPExcel_Chart_DataSeriesValues('String', 'Worksheet!$' . $legendCol . '$' . $firstDataRow .':$' . $legendCol . '$' . ($firstDataRow + $numDataRows - 1), null, 4),
		);
		...
		//		Data values
		//		Data Marker
			$dataSeriesValues[] = new PHPExcel_Chart_DataSeriesValues('Number', 'Worksheet!$' . $contChar . '$' . $firstDataRow .':$' . $contChar . '$' . ($firstDataRow + $numDataRows - 1), 
				null, 
				4);
		}
