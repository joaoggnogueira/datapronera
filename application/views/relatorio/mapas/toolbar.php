<ul class="nav navbar-nav scrollable selectable" id="nav-tools">
    ...
    <li class="dropdown">
        ...
        <ul class="dropdown-menu" id="filters-config">
            ...
            <li class="dropdown-submenu filter-type" value="tipo_instrumento">
                <a href="#" class="title-filter" count="<?= (is_bool($srAtual) ? "0" : count($superintendencias)) ?>">
                    Tipo de Instrumento
                </a>
                <ul class="dropdown-menu">
                    <li class="no-close fixed">
                        <input id="tipo_instrumento_all" class="select-all" <?= (is_bool($srAtual) ? "checked" : "indeterminate") ?> value="ALL" type="checkbox" />
                        <label href="#" for="tipo_instrumento_all">
                            Selecionar Tudo
                        </label>
                    </li>
                    <hr style="margin-bottom: 5px" />
                    <li>
                        <ul class="overflow">
                            <?PHP foreach ($tipo_instrumentos as $tipo) : ?>
                                <li class="no-close">
                                    <input class="filter-checkbox" id="tipo_instrumento_<?= $tipo->id ?>" checked value="<?= $tipo->id ?>" type="checkbox" />
                                    <label href="#" for="tipo_instrumento_<?= $tipo->id ?>">
                                        <?= $tipo->nome ?>
                                    </label>
                                </li>
                            <?PHP endforeach; ?>
            ...
            <li class="dropdown-submenu filter-type only-educando-filter" value="assentamento">
                ...
                <div class="dropdown-menu no-close" style="width: 350px;">
                    &nbsp;&nbsp;<i class="fa fa-exclamation-triangle"></i> Válido somente para o Mapa Educandos <div class="lds-ring" id="loading-assentamentos" style='display: none'>
                        <div></div>
                        <div></div>
                        <div></div>
                        <div></div>
...
<script>
    (function() {
        ...
        window.getFilter = function() {
            ...
            parent.find(".filter-type[value='tipo_instrumento'] input.filter-checkbox:checked").each(function() {
                data.tipo_instrumento.push(this.value);
            });
